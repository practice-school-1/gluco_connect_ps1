import { Controller, Get, Post, Patch, Delete, Param, Body, Query, UseGuards, Request, Injectable, NotFoundException, ForbiddenException, Module } from '@nestjs/common';
...
export class ActivitiesService {
  ...
  async findAll(userId: string, targetPatientId?: string) {
    ...
    if (targetPatientId) {
      ...
      if (!doctor) throw new ForbiddenException('Only doctors can view other patients\' activities');
      patient = await this.prisma.patient.findUnique({ where: { id: targetPatientId } });
      if (!patient || patient.doctor_id !== doctor.id) throw new ForbiddenException('Patient not linked to your account');
    } else {
      patient = await this.prisma.patient.findUnique({ where: { user_id: userId } });
    }
    if (!patient) throw new NotFoundException('Patient not found');
...
export class GlucoseService {
  ...
  async findAll(userId: string, targetPatientId?: string) {
    ...
    if (targetPatientId) {
      const doctor = await this.prisma.doctor.findUnique({ where: { user_id: userId } });
      if (!doctor) throw new ForbiddenException('Only doctors can view other patients\' readings');
      patient = await this.prisma.patient.findUnique({ where: { id: targetPatientId } });
      if (!patient || patient.doctor_id !== doctor.id) throw new ForbiddenException('Patient not linked to your account');
    } else {
      patient = await this.prisma.patient.findUnique({ where: { user_id: userId } });
  ...
  async getHistory(userId: string, days = 30, targetPatientId?: string) {
    let patient: any;
    if (targetPatientId) {
      const doctor = await this.prisma.doctor.findUnique({ where: { user_id: userId } });
      if (!doctor) throw new ForbiddenException('Only doctors can view other patients\' history');
      patient = await this.prisma.patient.findUnique({ where: { id: targetPatientId } });
      if (!patient || patient.doctor_id !== doctor.id) throw new ForbiddenException('Patient not linked to your account');
    } else {
      patient = await this.prisma.patient.findUnique({ where: { user_id: userId } });
    ...
    readings.forEach(r => {
      ...
      byDate[d].push(r.value_mg_dl);
    });

    return Object.entries(byDate).map(([date, vals]) => ({
      date,
      avg_glucose: Math.round(vals.reduce((a, b) => a + b, 0) / vals.length),
  ...
  async getTimeInRange(userId: string, days = 30) {
    const patient = await this.prisma.patient.findUnique({ where: { user_id: userId } });
    if (!patient) return { time_in_range_pct: 0, total_readings: 0 };

    const since = new Date();
    since.setDate(since.getDate() - days);

    const all = await this.prisma.glucoseReading.findMany({
      where: { patient_id: patient.id, recorded_at: { gte: since } },
      select: { value_mg_dl: true },
    });

    if (!all.length) return { time_in_range_pct: 0, total_readings: 0 };

    const min = patient.target_glucose_min ?? 70;
    const max = patient.target_glucose_max ?? 140;
    const inRange = all.filter(r => r.value_mg_dl >= min && r.value_mg_dl <= max).length;

    return {
      time_in_range_pct: Math.round((inRange / all.length) * 100),
      in_range: inRange,
      below_range: all.filter(r => r.value_mg_dl < min).length,
      above_range: all.filter(r => r.value_mg_dl > max).length,
      total_readings: all.length,
      target_min: min,
      target_max: max,
    };
  }
...
export class MealsService {
  ...
  async findAll(userId: string, targetPatientId?: string) {
    ...
    if (targetPatientId) {
      const doctor = await this.prisma.doctor.findUnique({ where: { user_id: userId } });
      if (!doctor) throw new ForbiddenException('Only doctors can view other patients\' meals');
      patient = await this.prisma.patient.findUnique({ where: { id: targetPatientId } });
      if (!patient || patient.doctor_id !== doctor.id) throw new ForbiddenException('Patient not linked to your account');
    } else {
      patient = await this.prisma.patient.findUnique({ where: { user_id: userId } });
...
export class ActivitiesController {
  ...
  findAll(@Request() req, @Query('patient_id') patientId?: string) {
    return this.activitiesService.findAll(req.user.userId, patientId);
  }
...
export class GlucoseController {
  ...
  findAll(@Request() req, @Query('patient_id') patientId?: string) {
    return this.glucoseService.findAll(req.user.userId, patientId);
  }

  @Get('history')
  @ApiOperation({ summary: 'Daily average glucose history for N days (pass patient_id for doctor access)' })
  history(@Request() req, @Query('days') days?: string, @Query('patient_id') patientId?: string) {
    return this.glucoseService.getHistory(req.user.userId, days ? parseInt(days) : 30, patientId);
  }

  @Get('time-in-range')
  @ApiOperation({ summary: 'Time-in-range percentage for N days' })
  timeInRange(@Request() req, @Query('days') days?: string) {
    return this.glucoseService.getTimeInRange(req.user.userId, days ? parseInt(days) : 30);
  }
...
export class MealsController {
  ...
  findAll(@Request() req, @Query('patient_id') patientId?: string) {
    return this.mealsService.findAll(req.user.userId, patientId);
  }
...
export class SummaryService {
  ...
  private async getPatient(userId: string) {
    const p = await this.prisma.patient.findUnique({ where: { user_id: userId } });
    if (!p) throw new NotFoundException('Patient profile required');
    return p;
  }
  ...
  async getDaily(userId: string, date?: string) {
    ...
    const [glucose, meals, activity] = await Promise.all([
      this.prisma.glucoseReading.findMany({
        where: { patient_id: patient.id, recorded_at: { gte: start, lte: end } },
        orderBy: { recorded_at: 'asc' },
      }),
      this.prisma.meal.findMany({
        where: { patient_id: patient.id, logged_at: { gte: start, lte: end } },
        include: { meal_items: true },
        orderBy: { logged_at: 'asc' },
      }),
      this.prisma.activity.findMany({
        where: { patient_id: patient.id, date: { gte: start, lte: end } },
      }),
    ]);

    const avgGlucose = glucose.length
      ? Math.round(glucose.reduce((s, r) => s + r.value_mg_dl, 0) / glucose.length)
      : null;
  ...
  async getWeekly(userId: string) {
    ...
    const [allGlucose, allActivities] = await Promise.all([
      this.prisma.glucoseReading.findMany({
        where: { patient_id: patient.id, recorded_at: { gte: since } },
        orderBy: { recorded_at: 'asc' },
      }),
    ...
    const byDay = days.map(date => {
      const g = allGlucose.filter(r => r.recorded_at.toISOString().slice(0, 10) === date);
      const a = allActivities.filter(act => act.date.toISOString().slice(0, 10) === date);
      return {
        date,
        avg_glucose: g.length ? Math.round(g.reduce((s, r) => s + r.value_mg_dl, 0) / g.length) : null,
        reading_count: g.length,
        total_steps: a.reduce((s, act) => s + (act.steps ?? 0), 0),
      };
    });

    const allVals = allGlucose.map(r => r.value_mg_dl);
    const min = patient.target_glucose_min ?? 70;
    const max = patient.target_glucose_max ?? 140;
    const inRange = allVals.filter(v => v >= min && v <= max).length;

    return {
      days: byDay,
      weekly_avg_glucose: allVals.length ? Math.round(allVals.reduce((a,b) => a+b,0) / allVals.length) : null,
      time_in_range_pct: allVals.length ? Math.round((inRange / allVals.length) * 100) : 0,
      total_readings: allVals.length,
      avg_daily_steps: Math.round(allActivities.reduce((s, a) => s + (a.steps ?? 0), 0) / 7),
    };
  }
}
...
export class SummaryController {
  ...
  @Get('daily')
  @ApiOperation({ summary: 'Combined daily summary: glucose + meals + activity' })
  daily(@Request() req, @Query('date') date?: string) {
    return this.summaryService.getDaily(req.user.userId, date);
  }

  @Get('weekly')
  @ApiOperation({ summary: 'Last 7 days summary with per-day breakdown' })
  weekly(@Request() req) {
    return this.summaryService.getWeekly(req.user.userId);
  }
}
